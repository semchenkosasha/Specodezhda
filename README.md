...
Оценка качества программного кода является важной частью обеспечения надёжности, сопровождаемости и расширяемости программных систем. В Java и фреймворка Spring Boot используются различные количественные и структурные метрики, позволяющие выявлять потенциальные проблемы архитектуры, сложности реализации и поддерживаемости проекта. Эти метрики служат инструментом для объективного анализа внутреннего состояния кода, определения его соответствия принципам хорошего проектирования и предупреждения дефектов на ранних этапах разработки.

Одной из ключевых метрик является цикломатическая сложность, измеряющая количество независимых путей выполнения в методах и классах. В контексте Spring Boot она особенно важна из-за обилия сервисных компонентов, контроллеров и конфигурационных классов, в которых высокая сложность может привести к ошибкам в логике обработки запросов, усложнению тестирования и снижению устойчивости системы к изменениям. Низкое значение метрики указывает на простую и хорошо структурированную логику, тогда как высокое значение требует рефакторинга и декомпозиции методов.

Другой значимой метрикой является уровень покрытия кода тестами, позволяющий оценить степень проверенности отдельных модулей посредством unit- и интеграционных тестов. Для приложений на Spring Boot, где активное использование зависимостей, контейнера инверсии управления и REST-контроллеров увеличивает количество взаимодействий между компонентами, высокий уровень покрытия тестами обеспечивает раннее обнаружение ошибок и стабильность приложения при модификациях. Эта метрика отражает зрелость процесса тестирования и помогает поддерживать качество системы в долгосрочной перспективе.

Третьей метрикой является оценка структурной согласованности слоёной архитектуры REST-API, основанной на соблюдении принципа единственной ответственности (SRP) и корректном разделении логики между слоями controller, service и repository. Данная метрика позволяет определить, насколько чётко разграничены обязанности компонентов и отсутствует ли смешивание функций, которое усложняет сопровождение и ведёт к дублированию кода. Анализ согласованности архитектуры показывает, выдерживается ли требуемая иерархия, не содержит ли контроллер бизнес-логики, а сервис – операций доступа к данным. Высокое значение метрики свидетельствует о чистой архитектуре, ясной структуре приложения и упрощённой поддержке проекта.

Для оценки качества разработанного программного средства были применены метрики, определённые на предыдущем этапе исследования. Анализ был выполнен с использованием специализированных инструментов статического анализа кода и модулей автоматизированного тестирования, входящих в Java и фреймворка Spring Boot. В процессе исследования оценивались структурные характеристики, сложность реализации и степень тестового покрытия, что позволило получить комплексное представление о текущем состоянии проекта и выявить потенциальные направления для улучшения архитектуры. 

Первым этапом анализа стала оценка цикломатической сложности. Среднее значение цикломатической сложности по сервисному слою составило 4,2, тогда как контроллеры показали более высокое значение – 6,1, что объясняется наличием разветвлённой логики обработки HTTP-запросов. Максимальное значение сложности, зафиксированное у одного из методов, достигло 11, что указывает на необходимость дальнейшего рефакторинга посредством декомпозиции и введения вспомогательных компонентов. В целом значения находятся в допустимых пределах, однако отдельные участки требуют оптимизации.

Второй метрикой стал показатель покрытия кода тестами. Для его получения использовался инструмент JaCoCo, интегрированный в процесс сборки Maven. В результате анализа установлено, что уровень покрытия unit-тестами составляет 10 %, а интеграционными тестами – 10 %. 

Заключительным этапом стала оценка корректности структурирования слоёной архитектуры REST-API. Анализ проводился с использованием встроенных инструментов IntelliJ IDEA, что позволило выявить нарушения принципа единственной ответственности (SRP) и определить степень соблюдения проектного разделения на controller, service и repository слои. Результаты показали, что классы следует слоенной модели: контроллеры содержат только логику обработки запросов, а сервисы – бизнес-логику. В репозиториях серьёзных нарушений не выявлено. В целом структура архитектуры оценивается как удовлетворительная.


### Документация

Ссылка на файл с документацией для Postman: https://github.com/semchenkosasha/Specodezhda/blob/main/api-docs.json
...
Тестовый метод testFindAll() предназначен для проверки корректности работы контроллера при получении всех элементов через REST API. На первом этапе выполняется HTTP-запрос типа GET к эндпоинту /workwears. После выполнения запроса проверяется, что статус ответа равен 200 OK, что подтверждает успешную обработку запроса. Далее осуществляется валидация структуры возвращаемого JSON: поле code должно содержать значение SUCCESS, а поле message – текст "Success Find All". Таким образом, тест подтверждает корректность работы контроллера при запросе всех элементов и правильность формирования ответа сервера.

```
@Test
    void testFind() throws Exception {
        Long id = 1L;
        mockMvc.perform(get("/workwears/{id}", id))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(StatusCode.SUCCESS))
...
Тестовый метод testFind() предназначен для проверки корректности работы контроллера при получении конкретного элемента по идентификатору через REST API. Выполняется HTTP-запрос типа GET к эндпоинту /workwears/{id}, где передаётся идентификатор элемента. После выполнения запроса проверяется, что статус ответа равен 200 OK, что подтверждает успешную обработку запроса. Далее осуществляется валидация структуры возвращаемого JSON: поле code должно содержать значение SUCCESS, а поле message – текст "Success Find". Таким образом, тест подтверждает корректность работы контроллера при запросе одного элемента и правильность формирования ответа сервера.
